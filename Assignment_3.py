...
fs = 5000
wc = 2*np.pi*1000 #cut off frequency
T = 1/fs
...
#Numerator
b = [(wc*T)/(T*wc+2),(wc*T)/(wc*T+2)]

#Frequency Response
w,h = sp.freqz(b,a)

plt.plot(w/np.pi/2*fs,np.abs(h))
###############################################################################
...
f0 = 0.1
w0 = np.pi*2*f0
r = 0.9 #Resonance try r = 0.99

a = [1,-2*r*np.cos(w0),r**2] #Denominator
b = [1,-2*np.cos(w0),1] #Numerator

w,h = signal.freqz(b,a) #Python Command
...
b,a = signal.butter[4,0.1*2]
sos = signal.butter(4,20,0.1*2,output='sos')
...
b,a = signal.cheby1[4,10,0.1*2] #10 is decibel
sos = signal.cheby1(4,10,0.1*2,output='sos')
...
b,a = signal.cheby2[4,40,0.1*2] #10 is decibel #change the order 4 to 6
sos = signal.cheby2(4,40,0.1*2,output='sos') #change the order 4 to 6
...
'''Class for 1st order IIR filter'''

class IIR_filter:
    def _init_(self,_b,_a):
        self.a = _a
        self.b = _b
        self.buffer = 0
        
    def filter(self,x):
        y = x*self.b - self.buffer*self.a
        self.buffer = y
        return y
...
#Coefficient
b1 = np.conj(si)/(np.conj(si)-si)
a1 = -np.exp(np.conj(si))
...
x = np.zeros(100)
x[10] = 1
y1 = np.zeros(100,dtype=complex)
y2 = np.zeros(100,dtype=complex)
